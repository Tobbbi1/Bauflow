-- Bauflow Database Schema - Vollständige Version
-- Mit Mitarbeiter-System, erweiterten Aufgaben und Kalender-Funktionalität
...
BEGIN
  ...
  INSERT INTO public.profiles (id, first_name, last_name, role, company_id, email_verified)
  VALUES (
    new.id,
...
-- Sample admin user (wird über die Registrierung erstellt)
-- INSERT INTO auth.users (email, encrypted_password, email_confirmed_at, created_at, updated_at, raw_user_meta_data)
-- VALUES ('admin@muster-handwerk.de', crypt('admin123', gen_salt('bf')), NOW(), NOW(), NOW(), 
--         '{"firstName": "Admin", "lastName": "User", "role": "admin", "companyId": "COMPANY_ID_HERE"}');
